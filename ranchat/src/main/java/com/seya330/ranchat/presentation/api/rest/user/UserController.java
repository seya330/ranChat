...
public class UserController {
  ...
  RegUserApplicationService userApplicationService;
	...
	public Object add(@RequestBody RegUserVO userVO) {
		...
		if(!userApplicationService.addUser(userVO)) {
	...
	public Object isOverlap(@PathVariable String userId) {
		...
		return ResponseEntity.status(result).body(userApplicationService.isOverlap(userVO));
	}
	
	@PostMapping("/login")
	public Object login(@RequestBody RegUserVO userVO) {
		HttpStatus result = HttpStatus.OK;
		LoginResultType resultType = userApplicationService.loginToRanchat(userVO);
	...
	public Object search(HttpServletRequest request, RegUserVO regUserVO) {
		...
		return ResponseEntity.status(HttpStatus.OK).body(userApplicationService.searchUser(regUserVO));
	}
}
