...
import com.seya330.ranchat.application.user.converter.RegUserVoConverter;
import com.seya330.ranchat.core.user.domain.RegUser;
import com.seya330.ranchat.core.user.domain.UserStatus;
import com.seya330.ranchat.core.user.exception.UserNotFoundException;
import com.seya330.ranchat.core.user.repository.RegUserRepository;
...
public class RegUserApplicationService {

  private final RegUserDAO userDAO;
  ...
  public String getToken(LoginCommand loginCommand) {
    RegUser regUser = regUserRepository.findByUserIdAndUserStatus(
        loginCommand.getUserId(),
        UserStatus.U)
        .orElseThrow(UserNotFoundException::new);

    regUser.validatePassword(loginCommand.getPassword());

    return jwtUtil.userBeanToJwtToken(RegUserVoConverter.INSTANCE.toRegUserVo(regUser));
  }
